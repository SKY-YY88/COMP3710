...
for i in range(epoch):
    ...
    # im_tb_list = glob(test_image+"/.*.png")
    # im_label_list = glob(test_label+"/.*.png")
    im_tb_list = sorted(os.listdir(test_image))
    im_label_list = sorted(os.listdir(test_label))
    print(im_tb_list[0])
    print(im_tb_list[1])
    im_tb = Image.open(test_image+im_tb_list[1])#.convert("RGB")
    im_label = Image.open(test_label+im_label_list[1])#.convert("RGB")
   ...
   #pred_tb = threshold_predictions_v(pred_tb)
    ...
    print("train_idx: ", len(train_idx), "valid_idx: ", len(valid_idx))
 ...
 #       writer1.add_scalar('Train Loss', train_loss, n_iter)
  #      writer1.add_scalar('Validation Loss', valid_loss, n_iter)
    ...
    img = Image.open(test_image+im_tb_list[1])#.convert("RGB")
